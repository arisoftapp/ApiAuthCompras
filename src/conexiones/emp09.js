...
var connectionString = "DSN=SELECTA09;UID=system;PWD=manager;DATABASE=EMP09";
...
setTimeout(() => {
    if (db.connected == false) {
        console.log("open cerrada reiniciar EMP09");
        process.exit(0);
    }
    else {
...
db.open(connectionString, function (err) {
    if (err) {
        console.log('SERVIDOR MACROPRO NO RESPONDE - VERIFIQUE QUE ESTE ENCENDIDO EMP09');
        process.exit(0);
    }
    else {
        let ts = Date.now();
