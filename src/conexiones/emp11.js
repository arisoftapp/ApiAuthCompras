...
var connectionString = "DSN=SELECTA11;UID=system;PWD=manager;DATABASE=EMP11";
...
setTimeout(() => {
    if (db.connected == false) {
        console.log("open cerrada reiniciar EMP11");
        process.exit(0);
    }
    else {
...
db.open(connectionString, function (err) {
    if (err) {
        console.log('SERVIDOR MACROPRO NO RESPONDE - VERIFIQUE QUE ESTE ENCENDIDO EMP11');
        process.exit(0);
    }
    else {
        let ts = Date.now();
