...
var connectionString = "DSN=SELECTA20;UID=system;PWD=manager;DATABASE=EMP21";
...
setTimeout(() => {
    if (db.connected == false) {
        console.log("open cerrada reiniciar EMP20");
        process.exit(0);
    }
    else {
...
db.open(connectionString, function (err) {
    if (err) {
        console.log('SERVIDOR MACROPRO NO RESPONDE - VERIFIQUE QUE ESTE ENCENDIDO EMP20');
        process.exit(0);
    }
    else {
        let ts = Date.now();
