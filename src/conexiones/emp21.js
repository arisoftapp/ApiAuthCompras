...
var connectionString = "DSN=SELECTA21;UID=system;PWD=manager;DATABASE=EMP22";
...
setTimeout(() => {
    if (db.connected == false) {
        console.log("open cerrada reiniciar EMP21");
        process.exit(0);
    }
    else {
...
db.open(connectionString, function (err) {
    if (err) {
        console.log('SERVIDOR MACROPRO NO RESPONDE - VERIFIQUE QUE ESTE ENCENDIDO EMP21');
        process.exit(0);
    }
    else {
        let ts = Date.now();
