...
var connectionString = "DSN=SELECTA22;UID=system;PWD=manager;DATABASE=EMP23";
...
setTimeout(() => {
    if (db.connected == false) {
        console.log("open cerrada reiniciar EMP22");
        process.exit(0);
    }
    else {
...
db.open(connectionString, function (err) {
    if (err) {
        console.log('SERVIDOR MACROPRO NO RESPONDE - VERIFIQUE QUE ESTE ENCENDIDO EMP22');
        process.exit(0);
    }
    else {
        let ts = Date.now();
