...
var connectionString = "DSN=SELECTA23;UID=system;PWD=manager;DATABASE=EMP24";
...
setTimeout(() => {
    if (db.connected == false) {
        console.log("open cerrada reiniciar EMP23");
        process.exit(0);
    }
    else {
...
db.open(connectionString, function (err) {
    if (err) {
        console.log('SERVIDOR MACROPRO NO RESPONDE - VERIFIQUE QUE ESTE ENCENDIDO EMP23');
        process.exit(0);
    }
    else {
        let ts = Date.now();
