...
var connectionString = "DSN=SELECTA24;UID=system;PWD=manager;DATABASE=EMP25";
...
setTimeout(() => {
    if (db.connected == false) {
        console.log("open cerrada reiniciar EMP24");
        process.exit(0);
    }
    else {
...
db.open(connectionString, function (err) {
    if (err) {
        console.log('SERVIDOR MACROPRO NO RESPONDE - VERIFIQUE QUE ESTE ENCENDIDO EMP24');
        process.exit(0);
    }
    else {
        let ts = Date.now();
